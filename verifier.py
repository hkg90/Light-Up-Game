...
# Function determines if any light bulb's placement overlaps
# with the beam from another light bulb
def check_beams(board, cell):
    ...
    # Check if light bulb placement is valid
    while True:
        y_index += 1
...
# Function ensures that each numbered black box has appropriate number of directly
...
def verifier(board, lightBulbCells, numberedBlackBox):
    for row in board:
        for cell in row:
            ...
            if cell.state == 0:
                return False
            # Check if multiple light bulbs exist within the same beam (row and/ or column of beams)
            if cell.state == 1:
                if not check_beams(board, [cell.x, cell.y]):
                    return False
            # Ensure that each numbered black box in board has appropriate number of directly
            # adjacent light bulbs
