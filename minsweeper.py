...
im          = ['', blank, bomb,flag, '']
color       = [('grey', 'red'), ('red', 'yellow'),
               ('red', 'red'), ('red', 'red'), ('gray', 'black')]
...
def new_card():                     # refresh all cells to hidden card - the cells that users can click on
    for x in range(width):
        for y in range(height):
            b[x][y].state = 1
            b[x][y].num = bomb[x][y]
...
def show_blank():                   # Show all cell not around by any bomb - make them gray blank squares
